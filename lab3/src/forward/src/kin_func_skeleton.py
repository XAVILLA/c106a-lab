...
def rotation_3d(omega, theta):
    ...
    result = np.zeros((3, 3))
    result[0, 0], result[1, 1], result[2, 2] = 1, 1, 1
    w_norm = (np.sum(omega**2))**(1/2)
    w_hat = skew_3d(omega)
    result += (w_hat/w_norm)*np.sin(w_norm*theta)

    result += (np.dot(w_hat,w_hat)/(w_norm**2))*(1-np.cos(w_norm*theta))
    # print(result)
    return result
...
def homog_3d(xi, theta):
    ...
    if np.allclose(w, 0):
        ...
        w_hat = skew_3d(w)
        a = np.dot((I-R), np.dot(w_hat,v))
        w = w.reshape(3, 1)
        b = np.dot(np.dot(w,w.T),v*theta)
        p = (1/w_norm**2)*(a+b)
    g = np.eye(4)
    g[:3, :3] = R
    g[:3, 3] = p
